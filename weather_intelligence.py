...
class WeatherIntelligence:
    def __init__(self):
        ...
        self.rain_effects = {
            'dry': {'sales_impact': 13.0, 'explanation': 'Курьеры работают активно'},
            'light_rain': {'sales_impact': 9.6, 'explanation': 'Клиенты дома, курьеры работают'},
            'moderate_rain': {'sales_impact': -10.1, 'explanation': 'Курьеры отказываются'},
            'heavy_rain': {'sales_impact': -6.5, 'explanation': 'Меньше влияния чем ожидалось'}
        }
        
        self.wind_effects = {
            'light_wind': {'sales_impact': 5.8, 'explanation': 'Нормальные условия'},
            'moderate_wind': {'sales_impact': -23.8, 'explanation': 'КРИТИЧНО: опасно для байкеров'}
        }
    ...
    def analyze_rain_impact(self, rain_mm):
        ...
        if rain_mm <= 0.1:
            ...
            category = "Сухо"
            severity = "Отлично"
            courier_status = "Все курьеры работают"
        elif rain_mm <= 2:
            effect = self.rain_effects['light_rain']
            category = "Легкий дождь"
            severity = "Хорошо"
            courier_status = "Курьеры работают, клиенты дома"
        elif rain_mm <= 8:
            effect = self.rain_effects['moderate_rain']
            category = "Умеренный дождь"
            severity = "Проблематично"
            courier_status = "Курьеры начинают отказываться"
        else:
            effect = self.rain_effects['heavy_rain']
            category = "Сильный дождь"
            severity = "Сложно"
            courier_status = "Ограниченная доступность курьеров"
    ...
    def analyze_wind_impact(self, wind_kmh):
        ...
        if wind_kmh <= 10:
            ...
            courier_impact = "Нормальные условия работы"
        elif wind_kmh <= 20:
            impact = 5.8
            category = "Легкий ветер"
            danger_level = "Безопасно"
            courier_impact = "Комфортные условия для курьеров"
        elif wind_kmh <= 30:
            impact = -23.8
            category = "Умеренный ветер"
            danger_level = "⚠️ КРИТИЧНО"
            courier_impact = "ОПАСНО для байкеров! Многие отказываются работать"
        else:
            impact = -30
            category = "Сильный ветер"
            danger_level = "🚨 ЭКСТРЕМАЛЬНО"
            courier_impact = "Крайне опасно, большинство курьеров не работает"
