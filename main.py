...
def generate_market_insights(market_data, leaders_df):
    ...
    if total_sales > 1000000000000:  # 1 триллион
        ...
        # Развивающийся рынок (менее 500 млрд IDR)
        total_sales_billions = total_sales / 1000000000
        potential_growth = 500 - total_sales_billions  # До "среднего рынка"
        insights.append(f"   💡 РАЗВИВАЮЩИЙСЯ РЫНОК: Потенциал для роста ({total_sales_billions:.0f} млрд IDR)")
        insights.append(f"   📊 Критерий: Оборот менее 500 млрд IDR = развивающийся рынок")
        insights.append(f"   🚀 Потенциал роста: +{potential_growth:.0f} млрд IDR до статуса 'средний рынок'")
