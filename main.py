...
def generate_market_insights(market_data, leaders_df):
    ...
    # Оценка размера рынка (относительная)
    total_sales_billions = total_sales / 1000000000
    insights.append(f"   💰 РАЗМЕР РЫНКА: {total_sales_billions:.0f} млрд IDR")
    insights.append(f"   📊 ОЦЕНКА: Локальный сегмент ресторанного рынка Индонезии")
    insights.append(f"   📈 КОНТЕКСТ: Общий рынок foodservice Индонезии ~950 трлн IDR (2025)")
    insights.append(f"   🎯 ДОЛЯ: Анализируемый сегмент составляет ~{(total_sales_billions / 950000):.3f}% от общего рынка")
