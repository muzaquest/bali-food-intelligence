...
def detect_sales_anomalies_and_causes(restaurant_data, weather_data, start_date, end_date):
    ...
    try:
        # Используем ML-анализ
        from ml_detective_analysis import MLDetectiveAnalyzer
        
        analyzer = MLDetectiveAnalyzer()
        restaurant_name = restaurant_data.iloc[0].get('restaurant_name', 'Ika Canggu')
        
        # Генерируем ML-анализ
        ml_results = analyzer.analyze_restaurant_anomalies(restaurant_name, start_date, end_date)
        
        if ml_results:
            insights.extend(ml_results.split('\n'))
        else:
