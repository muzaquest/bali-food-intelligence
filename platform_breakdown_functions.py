...
⚠️ МЕТОДИКА: 
• GRAB: только прямые продажи от рекламных кампаний
• GOJEK: продажи в период активной рекламы  
• Органические продажи (без рекламы) не учитываются в ROAS
"""
...
def generate_methodology_note(metric_type):
    ...
    methodologies = {
        'roas': 'Обе платформы используют точные ads_sales из API (проверено клиентом)',
        'conversion': 'Детальная воронка доступна только для GRAB (показы, клики, конверсии)',
        'demographics': 'Демографические данные не предоставляются платформами',
