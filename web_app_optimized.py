...
def ask_ai_question(question, context=""):
    """Обработка AI вопроса (заглушка для демонстрации)"""
    # В реальной реализации здесь будет вызов OpenAI API
    return f"""
🤖 **AI Ответ на ваш вопрос:**

**Вопрос:** {question}

**Анализ:**
Это отличный вопрос! На основе имеющихся данных я могу предположить следующее:

1. **Основные факторы влияния:**
   - Погодные условия (дождливые дни могут снижать продажи на 15-25%)
   - Праздники и сезонность туризма
   - Эффективность маркетинговых кампаний
   - Операционные проблемы (закрытие ресторана, недоступность товаров)

2. **Рекомендации:**
   - Проанализируйте детальный отчет ресторана за проблемный период
   - Сравните с рыночными показателями
   - Обратите внимание на внешние факторы (погода, праздники)

**💡 Совет:** Запустите детальный анализ ресторана для получения точных данных с ML-прогнозами.

*Примечание: Это демо-версия AI помощника. В полной версии будет интеграция с OpenAI GPT-4.*
"""
