...
class WeatherIntelligence:
    def __init__(self):
        """
        Инициализация с научно обоснованными коэффициентами
        
        ⚠️ ВАЖНО: Коэффициенты дождя исправлены 26.01.2025
        - Устранен статистический выброс (Prana: 43M IDR)
        - Проанализировано 99 дней с реальной погодой
        - Логика: чем сильнее дождь → тем больше потери
        """
        ...
        # ⚠️ КРИТИЧЕСКИ ВАЖНО: НЕ ИЗМЕНЯТЬ БЕЗ ДЕТАЛЬНОГО АНАЛИЗА!
        # Коэффициенты исправлены 26.01.2025 после обнаружения статистического выброса
        # Основано на анализе 99 дней с реальной погодой (mega_analyzer.py)
        # Проверено детально через rain_checker.py
        self.rain_effects = {
            'dry': {'sales_impact': -9.2, 'explanation': 'Люди выходят из дома, меньше заказов'},
            'light_rain': {'sales_impact': 18.1, 'explanation': 'ЛУЧШИЙ ЭФФЕКТ: клиенты дома, курьеры работают'},
            'moderate_rain': {'sales_impact': -16.7, 'explanation': 'Курьеры начинают отказываться'},
            'heavy_rain': {'sales_impact': -26.6, 'explanation': 'КРИТИЧНО: курьеры не работают, большие потери'}
        }
        
        self.wind_effects = {
            'calm': {'sales_impact': 8.5, 'explanation': 'Комфортные условия для курьеров'},
            'light_wind': {'sales_impact': -5.2, 'explanation': 'Легкие сложности для байкеров'},
            'moderate_wind': {'sales_impact': -12.8, 'explanation': 'Заметные сложности доставки'}
        }
    ...
    def analyze_rain_impact(self, rain_mm):
        ...
        if rain_mm <= 0.1:
            ...
            category = "Сухо"
            severity = "Нормально"
            courier_status = "Люди выходят из дома, меньше заказов"
        elif rain_mm <= 2:
            ...
            severity = "⚠️ Проблематично"
            courier_status = "Курьеры начинают отказываться от заказов"
        else:
            effect = self.rain_effects['heavy_rain']
            category = "Сильный дождь"
            severity = "🚨 КРИТИЧНО"
            courier_status = "Большинство курьеров не работает, критические потери"
    ...
    def analyze_wind_impact(self, wind_kmh):
        ...
        if wind_kmh <= 10:
            impact = 8.5
            category = "Штиль"
            danger_level = "🌟 КОМФОРТНО"
            courier_impact = "Комфортные условия для курьеров, легкая работа"
        elif wind_kmh <= 20:
            impact = -5.2
            category = "Легкий ветер"
            danger_level = "✅ Нормально"
            courier_impact = "Легкие сложности для байкеров, но работают нормально"
        elif wind_kmh <= 30:
            impact = -12.8
            category = "Умеренный ветер"
            danger_level = "⚠️ Сложно"
            courier_impact = "Заметные сложности доставки, снижение эффективности"
        else:
            impact = -25.0
